...
import { EarningBalance, Hawala, Order } from '@/types/interface';
...
import { _fetchEarningBalanceRequestList } from '@/app/redux/actions/earningBalanceActions';
import { earningBalanceReducer } from '../../../redux/reducers/earningBalanceReducer';
...
const EarningBalanceRequest = () => {
    ...
    const { earningBalances } = useSelector((state: any) => state.earningBalanceReducer);
    ...
    useEffect(() => {
        ...
        dispatch(_fetchEarningBalanceRequestList(1, searchTag));
    ...
    const amountBodyTemplate = (rowData: EarningBalance) => {
        return (
            <>
                ...
                <span style={{ fontSize: '0.8rem', color: '#666' }}>{rowData.amount}</span>
            </>
        );
    };
    ...
    const reviewedByBodyTemplate = (rowData: EarningBalance) => {
        return (
            <>
                ...
                <span style={{ fontSize: '0.8rem', color: '#666' }}>{rowData.reviewed_by}</span>
            </>
        );
    };
        ...
        return (
            <>
                <span className="p-column-title">Reviewed At</span>
                <span style={{ fontSize: '0.8rem', color: '#666' }}>{rowData.reviewed_at}</span>
            </>
        );
    };
            ...
            <div className="col-12">
                <div className="card p-2">
                    ...
                    >
                        ...
                        <Column style={{ ...customCellStyle, textAlign: ['ar', 'fa', 'ps', 'bn'].includes(i18n.language) ? 'right' : 'left' }} field="" header={t('EARNING_BALANCE_REQUEST.TABLE.COLUMN.RESELLER')} body={resellerNameBodyTemplate}></Column>

                        <Column style={{ ...customCellStyle, textAlign: ['ar', 'fa', 'ps', 'bn'].includes(i18n.language) ? 'right' : 'left' }} field="status" header={t('EARNING_BALANCE_REQUEST.TABLE.COLUMN.STATUS')} body={statusBodyTemplate} sortable></Column>
                    </DataTable>
                    <Paginator
